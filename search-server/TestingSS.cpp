...
void TestMinusWords()
{
    ...
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::seq, "кот -белый"s);
        ASSERT_EQUAL(found_docs.size(), 1);
        const Document& doc0 = found_docs[0];
        ASSERT_EQUAL(doc0.id, 1);
    }
    ...
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::par, "кот -белый"s);
        ASSERT_EQUAL(found_docs.size(), 1);
        const Document& doc0 = found_docs[0];
        ASSERT_EQUAL(doc0.id, 1);
    }
...
void TestDocumentMatching()
{
    ...
    {
        const vector <string_view> query_test = { "евгений"sv, "скворец"sv };
    ...
    {
        const vector <string_view> query_test = { "пёс"sv, "ухоженный"sv };
        const auto& [query, docs_status] = search_server.MatchDocument("   ухоженный пёс   "s, 1);
        ASSERT_EQUAL(docs_status, DocumentStatus::ACTUAL);
        ASSERT(query.empty());
    }

    // проверяем работу "параллельной версии" проверки соответствия документа поисковому запросу 
    {
        const vector <string_view> query_test = { "евгений"sv, "скворец"sv };
        const auto& [query, docs_status] = search_server.MatchDocument(std::execution::par, "скворец     евгений   "s, 3);
        ASSERT_EQUAL(docs_status, DocumentStatus::BANNED);
        ASSERT_EQUAL(query, query_test);
    }

    // проверяем работу "параллельной версии" проверки соответствия документов поисковому запросу при несоответствии
    {
        const vector <string> query_test = { "пёс"s, "ухоженный"s };
        const auto& [query, docs_status] = search_server.MatchDocument(std::execution::par, "   ухоженный пёс   "s, 1);
        ASSERT_EQUAL(docs_status, DocumentStatus::ACTUAL);
        ASSERT(query.empty());
    }
...
void TestSortByRelevance()
{
    ...
    //Снова ищем котов, но с sequence политикой 
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::seq, "кот"s);
        ASSERT_EQUAL(found_docs.size(), 5);
        double relevance = found_docs[0].relevance;
        for (const Document& doc : found_docs)
        {
    ...
    //а теперь с parallel политикой 
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::par, "кот"s);
        ASSERT_EQUAL(found_docs.size(), 5);
        double relevance = found_docs[0].relevance;
        for (const Document& doc : found_docs)
        {
...
void TestRelevanceSearchihgDocuments()
{
    ...
    // проверяем ранжирование и расчёт по релевантности
    {
        ...
        relevavce[0] = doc0.relevance + doc1.relevance;
    }

    // проверяем ранжирование и расчёт по релевантности seq
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::seq, "кот"s);
        ASSERT_EQUAL(found_docs.size(), 2);
        const Document& doc0 = found_docs[0];
        ...
        ASSERT(abs(doc0.relevance - log(search_server.GetDocumentCount() * 1.0 / 2) * (1.0 / 4)) < 1e-6);
        ASSERT(abs(doc1.relevance - log(search_server.GetDocumentCount() * 1.0 / 2) * (1.0 / 5)) < 1e-6);
        relevavce[1] = doc0.relevance + doc1.relevance;
    }

    // проверяем ранжирование и расчёт по релевантности par
    {
        const auto found_docs = search_server.FindTopDocuments(std::execution::par, "кот"s);
        ASSERT_EQUAL(found_docs.size(), 2);
        const Document& doc0 = found_docs[0];
        ...
        ASSERT(abs(doc0.relevance - log(search_server.GetDocumentCount() * 1.0 / 2) * (1.0 / 4)) < 1e-6);
        ASSERT(abs(doc1.relevance - log(search_server.GetDocumentCount() * 1.0 / 2) * (1.0 / 5)) < 1e-6);
        relevavce[2] = doc0.relevance + doc1.relevance;
    }
    ASSERT_EQUAL(relevavce[0], relevavce[1]);
    ASSERT_EQUAL(relevavce[0], relevavce[2]);
}
...
void TestGetWordFrequencies()
{
    ...
    search_server.AddDocument(10, "ухоженный пёс выразительные глаза"s, DocumentStatus::ACTUAL, { 5, -12, 2, 1 });
    search_server.AddDocument(20, "ухоженный скворец евгений"s, DocumentStatus::BANNED, { 9 });
    const map <string_view, double> wfd = search_server.GetWordFrequencies(10);
    ...
    const map <string_view, double> wfd1 = search_server.GetWordFrequencies(5);
    ASSERT(wfd1.empty());    
}
