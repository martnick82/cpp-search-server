...
//Тренажёр принял решение, похоже сложность он не оценивает, и придётся всё переделывать
...
void SearchServer::RemoveDocument(int document_id)
{
    ...
    // Удаление слов, которые входили только в удаляемый документ
    for (auto it = word_to_document_freqs_.begin(); it != word_to_document_freqs_.end(); ++it)
    {
        if (it->second.size() == 0) // слова нет ни в одном документе
        {
